...
def latencies_to_csv(csv_target_directory, latencies, figure):
    cdf_data, cdf_log_data = latencies_to_cdfs(latencies)

    cdf_csv_file = os.path.join(csv_target_directory, '%s.csv' % figure)
    cdf_log_csv_file = os.path.join(csv_target_directory, '%s-log.csv' % figure)
